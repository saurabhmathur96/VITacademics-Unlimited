...
module.exports.parseHistory = html => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const gradesScraper = cheerio.load(
        baseScraper("table table")
          .eq(1)
          .html()
      );
      gradesScraper("tr").each((i, elem) => {
        ...
        const exam_held = moment(attrs.eq(6).text(), "MMM-YYYY").format(
          "YYYY-MM"
        );
        ...
        data.grades.push({
          ...
          grade: grade,
          exam_held: exam_held,
          result_date: moment(attrs.eq(7).text(), "DD-MMM-YYYY").isValid()
            ? moment(attrs.eq(7).text(), "DD-MMM-YYYY").format("YYYY-MM-DD")
            : null,
        ...
        if (gradeValue(grade) === true) {
          ...
          if (data.semester_wise[exam_held]) {
            data.semester_wise[exam_held].gpa =
              Math.round(
                ((data.semester_wise[exam_held].gpa *
                  data.semester_wise[exam_held].credits +
      ...
      const creditsTable = baseScraper("table table")
        .eq(2)
        .children("tr")
        .eq(1);
      ...
      data.credits_earned = parseInt(
        creditsTable
          .children("td")
          .eq(1)
          .text()
      );
      data.cgpa = parseFloat(
        creditsTable
          .children("td")
          .eq(2)
          .text()
      ...
      const gradeSummaryTable = baseScraper("table table")
        .eq(3)
        .children("tr")
        .eq(1);
      ...
      data.grades = data.grades.filter(
        grade => grade.credits != null && !isNaN(grade.credits)
      );
      return resolve(data);
    } catch (ex) {
      return reject(ex);
...
module.exports.parseGrades = html => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      var gpa = Number(
        baseScraper('span[style="font-size: 18px; font-weight: bold;"]')
          .text()
          .split(":")[1]
          .trim()
      );
      ...
      gradesScraper("tr").each((i, elem) => {
        ...
        if (credits > 0) credits_total = credits_total + credits;
