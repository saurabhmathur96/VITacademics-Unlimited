...
module.exports.parseReport = html => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const semcode = $("input[type=hidden][name='semcode']").map(
        (i, e) => e.attribs.value
      );
      const classnbr = $("input[type=hidden][name='classnbr']").map(
        (i, e) => e.attribs.value
      );
      const from_date = $("input[type=hidden][name='from_date']").map(
        (i, e) => e.attribs.value
      );
      const to_date = $("input[type=hidden][name='to_date']").map(
        (i, e) => e.attribs.value
      );
      const crscd = $("input[type=hidden][name='crscd']").map(
        (i, e) => e.attribs.value
      );
      const crstp = $("input[type=hidden][name='crstp']").map(
        (i, e) => e.attribs.value
      );
...
module.exports.parseDetails = html => {
  return new Promise((resolve, reject) => {
    try {
      const table = tabletojson
        .convert(html, { ignoreEmptyRows: true, allowHTML: false })[0]
        .slice(5);
