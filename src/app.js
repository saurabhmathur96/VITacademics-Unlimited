...
const curriculum = require(path.join(__dirname, "routes", "curriculum"));

const authentication = require(path.join(
  __dirname,
  "middleware",
  "authentication"
));
...
function onlyVellore(req, res, next) {
  if (req.body.campus !== "vellore") {
    const err = new Error(
      `This feature is not supported for the requested \`campus\`.`
    );
    err.status = 400;
    return next(err);
  } else {
...
app.use("/student/curriculum", curriculum);
