...
const supportedCampuses = ["vellore", "chennai"];
...
module.exports = (req, res, next) => {
  ...
  req
    .checkBody("semester", "`semester` not supported.")
    .optional()
    .isIn(supportedSemesters);
  req
    .checkBody("campus", "`campus` not supported.")
    .optional()
    .isIn(supportedCampuses);
  ...
  req
    ...
    .then(result => {
      if (!result.isEmpty()) {
        ...
        var portal = campus === "vellore" ? "vtopbeta" : "vtop";
        ...
        const key = crypto
          .createHash("md5")
          .update(portal + req.body.reg_no + req.body.password)
          .digest("hex");
        ...
        return signIn(req.body.reg_no, req.body.password, campus).then(
          cookies => {
