...
module.exports = (username, password, campus) => {
  ...
  return requests
    .getCookies(`${baseUri}/stud_login.asp`, null)
    .then(result => getCaptcha(result.cookies, campus))
    .then(result =>
      requests.postCookies(`${baseUri}/stud_login_submit.asp`, result.cookies, {
        regno: username,
        passwd: password,
        vrfcd: result.captcha
      })
    )
    .then(result => {
      try {
        const $ = cheerio.load(result.body);
        const welcomeMessage = $("table")
          .eq(1)
          .find("td")
          .eq(0)
          .text()
    ...
    .then(result =>
      requests.getCookies(`${baseUri}/stud_home.asp`, result.cookies)
    )
...
function getCaptcha(cookies, campus) {
  ...
  return new Promise((resolve, reject) => {
    ...
    cookies.forEach(cookie =>
      cookieJar.add(unirest.cookie(cookie), `${baseUri}/stud_login_submit.asp`)
    );

    unirest
      .get(`${baseUri}/captcha.asp`)
      .jar(cookieJar)
      .encoding(null)
      .timeout(26000)
      .end(response => {
